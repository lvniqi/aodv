#ifndef _KAODV_MOD_H_
#define _KAODV_MOD_H_
...
static rwlock_t ifilock;// = RW_LOCK_UNLOCKED;//kernel 3.18 don`t have
...
static inline int if_info_add(struct net_device *dev)
{
	...
	if(indev)
	{
		...
		{
			if(!strcmp(dev->name, ifa->ifa_label))//if find
				break;
		}
...
static inline int if_info_from_ifindex(struct in_addr *ifa, struct in_addr *bc, int ifindex)
{
