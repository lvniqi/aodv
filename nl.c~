...
static void nl_kaodv_callback(s32_t sock)
{
	...
	switch(nlm->nlmsg_type)
	{
		...
		case KAODVM_TIMEOUT:
			...
			printf("Got timeout msg form kernel for %s\n", inet_ntoa(dest_addr));
		...
		case KAODVM_ROUTE_REQ:
			...
			printf("Got ROUTE_REQ from kernel for %s\n", inet_ntoa(dest_addr));
		...
		case KAODVM_REPAIR:
			...
			printf("Got ROUTE_REPAIR from kernel for %s\n", inet_ntoa(dest_addr));
		...
		case KAODVM_SEND_RERR:
			...
			printf("Got SEND_RERR from kernel, S= %s, D= %s\n", inet_ntoa(src_addr), inet_ntoa(dest_addr));
			...
			do
			{
				...
				printf("Sending RERR for unsolicited message from %s to %s\n", inet_ntoa(src_addr), inet_ntoa(dest_addr));
				...
				if(wait_on_reboot)
				{
					printf("Wait on reboot timer reset\n");
		...
		case RTM_NEWADDR:
			...
			for(; RTA_OK(rta, attrlen); rta = RTA_NEXT(rta, attrlen))
			{
				if(rta->rta_type == IFA_ADDRESS)
				{
					...
					printf("Interface index %d changed address to %s\n", ifm->ifa_index, inet_ntoa(ifaddr));
				}
			}
			break;
		case RTM_NEWROUTE:
			...
			break;
		default:
			printf("Unkown type of kernel message!\n");
			break;
	}
}
...
s32_t nl_send_add_route_msg(struct in_addr dest, struct in_addr next_hop, s32_t metric, u32_t lifetime, s32_t rt_flags, s32_t ifindex)
{
	...
	printf("ADD/UPDATE: %s:%s ifindex= %d\n", inet_ntoa(dest), inet_ntoa(next_hop), ifindex);
...
s32_t nl_send_no_route_found_msg(struct in_addr dest)
{
	...
	printf("Send NOROUTE_FOUND to kernel: %s\n", inet_ntoa(dest));
...
s32_t nl_send_del_route_msg(struct in_addr dest, struct in_addr next_hop, s32_t metric)
{
	...
	printf("Send DEL_ROUTE to kernel: %s\n", inet_ntoa(dest));
...
s32_t nl_send_conf_msg(void)
{
	...
	printf("Send CONF_MSG to kernel\n");
